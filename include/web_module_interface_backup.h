...
private:
  ...
  static String baseCSS;        // Phase 6: Base CSS framework
  static String themeCSS;       // Phase 6: Theme overlay CSS
  static bool themeInitialized; // Phase 6: Track theme initialization
...
public:
  ...
  // NOTE: Legacy method - use modern theme system for new implementations
  ...
  // Modern theme system using base CSS framework and theme overlays
  // Simplified approach - no runtime theme switching needed
  static void initializeBaseTheme();
  static void setTheme(const String &baseCSS, const String &themeCSS = "");
  static std::vector<WebRoute> getThemeAssetRoutes();
  ...
  private:
    ...
    static std::map<int, String>
        errorPages; // Custom error pages by status code
    static std::vector<RedirectRule> redirectRules; // URL redirect rules
    static std::vector<StaticAsset>
        staticAssets; // Static assets (JS, images, etc.)
